...
GRADLE_VERSION=4.3
ACCEPT_LICENSE=${ANDROID_HOME}/tools/android-accept-licenses.sh
...
if [ ! -f ${ACCEPT_LICENSE} ]; then
  ...
  sudo cp -f $tmpFile ${ACCEPT_LICENSE}
  sudo chmod +x ${ACCEPT_LICENSE}
  ...
  pushd /opt
    ...
    sudo ${ACCEPT_LICENSE} "${ANDROID_HOME}/tools/android update sdk --all --no-ui --filter $FILTER"
